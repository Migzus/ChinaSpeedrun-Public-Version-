...
void cs::VulkanEngineRenderer::CreateCommandBuffers()
{
	...
	for (size_t i{ 0 }; i < commandBuffers.size(); i++)
	{
		...
		auto _renderers{ ChinaEngine::world.registry.view<MeshRendererComponent>() };
		auto _cameraComponent = ChinaEngine::world.registry.get<CameraComponent>(ChinaEngine::world.registry.view<CameraComponent>().front());
		...
		for (auto e : _renderers)
		{
			...
			// TEMP Solution
			_meshRenderer.ubo.proj = _cameraComponent.proj;
			_meshRenderer.ubo.view = _cameraComponent.view;
