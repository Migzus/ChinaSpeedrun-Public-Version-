...
public:
    ...
    /*
        Custom function by Jarle Kluge
    */
    bool convertPCMToBuffer(std::vector<uint8_t>& outBuffer);
...
private:
    ...
    bool decodeWaveFileHeader(std::vector<uint8_t>& fileData);
...
template <class T>
bool AudioFile<T>::convertPCMToBuffer(std::vector<uint8_t>& outBuffer) {
    ...
    for (int i = 0; i < getNumSamplesPerChannel(); i++)
    {
        for (int channel = 0; channel < getNumChannels(); channel++)
        {
            ...
            else if (bitDepth == 16)
            {
                ...
                addInt16ToFileData(outBuffer, sampleAsInt);
            }
            else if (bitDepth == 24)
            {
                ...
                outBuffer.push_back(bytes[0]);
                outBuffer.push_back(bytes[1]);
                outBuffer.push_back(bytes[2]);
            }
            else if (bitDepth == 32)
            {
                int32_t sampleAsInt;
                ...
                addInt32ToFileData(outBuffer, sampleAsInt, Endianness::LittleEndian);
            }
            else
            {
                assert(false && "Trying to convert samples with unsupported bit depth to buffer");
                return false;
            }
        }
    }
...
//=
template <class T>
bool AudioFile<T>::decodeWaveFileHeader(std::vector<uint8_t>& fileData)
{
    // -----------------------------------------------------------
    // HEADER CHUNK
    ...
    indexOfDataChunk = getIndexOfChunk(fileData, "data", 12);
    int indexOfFormatChunk = getIndexOfChunk(fileData, "fmt ", 12);
    indexOfXMLChunk = getIndexOfChunk(fileData, "iXML", 12);
    ...
    numBytesPerBlock = twoBytesToInt(fileData, f + 20);
    bitDepth = (int)twoBytesToInt(fileData, f + 22);

    numBytesPerSample = static_cast<uint16_t> (bitDepth) / 8;
...
{
    if (!decodeWaveFileHeader(fileData))
    {
        reportError("ERROR: couldn't decode wave file header");
        return false;
    }
